...
class MyProvider extends Component {
  ...
  componentDidMount = () => {
    this.getLowScore(this.startGame);
  };
      ...
      onCloseStart: () =>
        this.setState(
          ...
          () => {
            ...
            if (
              this.state.score === 12 &&
              this.state.wrong < this.state.lowScore
            ) {
              this.setState(
                {
  ...
  removeMatch = () => {
    const remainingFriends = this.state.friends.map(friend => {
      if (
        friend.id === this.state.chosen[0].id ||
        friend.id === this.state.chosen[1].id
      ) {
        friend = {
          ...friend,
          matched: true
      ...
      <MyContext.Provider
        value={{
          ...
          cardClickHandler: (e, id) => {
            e.preventDefault();
            const friendIndex = this.state.friends.findIndex(
              friend => friend.id === id
            );
